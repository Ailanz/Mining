<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ManageMenu.aspx.cs" Inherits="ManageMenu" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Manage Menu</title>
    <link rel="stylesheet" href="//netdna.bootstrapcdn.com/bootstrap/3.1.1/css/bootstrap.min.css" />
    <link rel="stylesheet" href="//netdna.bootstrapcdn.com/bootstrap/3.1.1/css/bootstrap-theme.min.css" />
    <script src="//ajax.googleapis.com/ajax/libs/jquery/2.1.0/jquery.min.js"></script>
    <script type="text/javascript" src="//netdna.bootstrapcdn.com/bootstrap/3.1.1/js/bootstrap.min.js"></script>
</head>

<body>
    <form id="form1" runat="server">
        <div class="container">
            <br />
            <asp:Panel runat="server" ID="panel">
                <asp:GridView runat="server" ID="menuItems" class="table table-hover" BorderWidth="0">
                </asp:GridView>
            </asp:Panel>
        </div>

        <asp:Panel ID="addServerPanel" Style="border-radius: 5px;" Width="350px" runat="server" CssClass="bg-primary container">
            <br />
            <p>
                <asp:TextBox ID="itemName" class="form-control" runat="server" placeholder="Item Name" />
            </p>
            <p>
                <asp:DropDownList ID="itemType" class="form-control" runat="server" placeholder="Type">
                    <asp:ListItem>Appetizer</asp:ListItem>
                    <asp:ListItem>Entree</asp:ListItem>
                    <asp:ListItem>Dessert</asp:ListItem>
                    <asp:ListItem>Drink</asp:ListItem>
                </asp:DropDownList>
            </p>
            <p>
                <asp:TextBox ID="price" class="form-control" runat="server" placeholder="Price (eg. 5.99)" Width="120" MaxLength="8" />
            </p>
            <p>
                <asp:TextBox ID="cost" class="form-control" runat="server" placeholder="Cost (eg. 5.99)" Width="120" MaxLength="8" />
            </p>
            <p>
                <asp:Button ID="submit" class="form-control" CssClass="btn btn-default" runat="server" Text="Enter Item" OnClick="submit_Click" />
            </p>

        </asp:Panel>
    </form>
</body>
</html>
